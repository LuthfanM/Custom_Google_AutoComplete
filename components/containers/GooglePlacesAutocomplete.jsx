...
import { setResult, setResults, setTextSearch } from '../../helpers/redux/reducers/general';
...
export const GooglePlacesAutocomplete = forwardRef((props, ref) => {
  ...
  const buildRowsFromResults = (results) => {
    ...
    if (results.length === 0 || props.predefinedPlacesAlwaysVisible === true) {
      ...
      if (props.currentLocation === true && props.hasNavigator) {
        res.unshift({
      ...
      request.onreadystatechange = () => {
        ...
        if (request.status === 200) {
          ...
          if (responseJSON.status === 'OK') {
            ...
            // let _temp = _renderDescription(rowData)
            Promise.resolve(setStateText(_renderDescription(rowData))).then(() => {
              dispatch(setTextSearch(stateText))
            })
